(function () {
  ...
  var Transaction = Dogecoin.Transaction = function (doc) {
    this.version = 1;
    this.lock_time = 0;
    this.ins = [];
    this.outs = [];
   ...
   */
  Transaction.prototype.addOutput = function (address, value, index) {
    if (arguments[0] instanceof TransactionOut) {
      ...
      } else if (Dogecoin.Util.isArray(value)) {
        // Nothing to do
      }
   ...
   *
   * Returns the transaction as a byte array in the standard Dogecoin binary
   * format. This method is byte-perfect, i.e. the resulting byte array can
   * be hashed to get the transaction's standard Dogecoin hash.
   */
  Transaction.prototype.serialize = function ()
  {
    var buffer = [];
    buffer = buffer.concat(Crypto.util.wordsToBytes([parseInt(this.version)]).reverse());
    buffer = buffer.concat(Dogecoin.Util.numToVarInt(this.ins.length));
    for (var i = 0; i < this.ins.length; i++) {
      ...
      buffer = buffer.concat(Dogecoin.Util.numToVarInt(scriptBytes.length));
      buffer = buffer.concat(scriptBytes);
      buffer = buffer.concat(Crypto.util.wordsToBytes([parseInt(txin.sequence)]).reverse());
    }
    buffer = buffer.concat(Dogecoin.Util.numToVarInt(this.outs.length));
   ...
   *
   * Dogecoin uses a different hash for each signed transaction input. This
  ...
  Transaction.prototype.analyze = function (wallet) {
    if (!(wallet instanceof Dogecoin.Wallet)) return null;
    ...
    if (impact.sign > 0 && impact.value.compareTo(BigInteger.ZERO) > 0) {
      analysis.type = 'recv';
      analysis.addr = new Dogecoin.Address(firstMeRecvHash);
      ...
      //       change address.
      analysis.addr = new Dogecoin.Address(firstRecvHash);
    } else  {
      analysis.type = "other";
    }
  ...
  Transaction.prototype.getTotalOutValue = function () {
    ...
    for (var j = 0; j < this.outs.length; j++) {
      ...
      totalValue = totalValue.add(Dogecoin.Util.valueToBigInt(txout.value));
    }
    return totalValue;
  };
  ...
  Transaction.prototype.calcImpact = function (wallet) {
    if (!(wallet instanceof Dogecoin.Wallet)) return BigInteger.ZERO;
    ...
    for (var j = 0; j < this.outs.length; j++) {
      ...
      if (wallet.hasHash(hash)) {
        valueOut = valueOut.add(Dogecoin.Util.valueToBigInt(txout.value));
      }
    }
    ...
    for (var j = 0; j < this.ins.length; j++) {
      ...
      if (wallet.hasHash(hash)) {
        ...
        if (fromTx) {
          valueIn = valueIn.add(Dogecoin.Util.valueToBigInt(fromTx.outs[txin.outpoint.index].value));
        }
      }
    }
  ...
  var TransactionIn = Dogecoin.TransactionIn = function (data)
  {
  ...
  var TransactionOut = Dogecoin.TransactionOut = function (data)
  {
    if (data.script instanceof Script) {
      this.script = data.script;
    } else {
    ...
    if (Dogecoin.Util.isArray(data.value)) {
