(function () {
  ...
  var Script = Dogecoin.Script = function (data) {
    if (!data) {
      this.buffer = [];
    } else if ("string" == typeof data) {
      this.buffer = Crypto.util.base64ToBytes(data);
    } else if (Dogecoin.Util.isArray(data)) {
      this.buffer = data;
   ...
   * Address:
   *   Paying to a Dogecoin address which is the hash of a pubkey.
  ...
  Script.prototype.simpleOutHash = function ()
  {
    ...
    case 'Pubkey':
      return Dogecoin.Util.sha256ripe160(this.chunks[0]);
    default:
  ...
  Script.prototype.getInType = function ()
  {
    if (this.chunks.length == 1 &&
        Dogecoin.Util.isArray(this.chunks[0])) {
      // Direct IP to IP transactions only have the signature in their scriptSig.
      // TODO: We could also check that the length of the data is correct.
      return 'Pubkey';
    } else if (this.chunks.length == 2 &&
               Dogecoin.Util.isArray(this.chunks[0]) &&
               Dogecoin.Util.isArray(this.chunks[1])) {
      return 'Address';
    } else {
      return 'Strange';
    }
  ...
  Script.prototype.simpleInPubKey = function ()
  {
    ...
    case 'Pubkey':
      ...
      //       See https://Dogecointalk.org/?topic=6430.0
  ...
  {
    return Dogecoin.Util.sha256ripe160(this.simpleInPubKey());
  };
  ...
  /**
   * Extract Dogecoin addresses from an output script
   */
  ...
  {
    var script = new Dogecoin.Script();
