...
(function () {
    ...
    sr.eventSeedLimit =  (function () {
		var num = Crypto.util.randomBytes(12)[11];
		return 200 + Math.floor(num);
	})();
    
    sr.lastEventSeedTime = new Date().getTime();
    ...
    sr.seedEvent = function(evt) {
        if (!evt) var evt = window.event;
        
        var timestamp = new Date().getTime();
        
        if(sr.eventSeedCount <= sr.eventSeedLimit && evt && (timestamp - sr.lastEventSeedTime) > 40) {
            sr.eventSeedCount++;
            sr.seedTime();
            sr.seedInt16((evt.clientX * evt.clientY));
            sr.lastEventSeedTime = new Date().getTime();
        }
    };
